...
export async function POST(request: Request) {
  try {
    const { domain, supabaseUrl: directUrl, supabaseKey: directKey, deepScanRequest, deep_scan_request_id } = await request.json();

    if (!domain && !directUrl) {
      return NextResponse.json({ 
        error: 'Either domain or supabaseUrl is required' 
      }, { status: 400 });
    }
